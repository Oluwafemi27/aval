# Web Rendered Motion Implementation Plan

**Date:** 2026-07-11

**Status:** Approved design moving into implementation

**Design:** [Web Rendered Motion Format Design](../specs/2026-07-11-web-rendered-motion-format-design.md)

## 1. Objective

Build the web-only rendered-motion system in evidence-driven milestones. Each milestone freezes only the contracts it proves. The first milestone is an in-memory WebCodecs experiment, not the final container: it must demonstrate that one independently decodable loop unit can be submitted repeatedly with new rational timestamps, without seeking, resetting, reconfiguring, or flushing at its seams.

The implementation remains unnamed. Package names use the private descriptive `@rendered-motion/*` scope and sample assets use the prototype `.rma` extension only until naming is decided.
...
Round-trip golden fixtures, mutation fuzzing, malformed samples, overlap/alias tests, and bounded-allocation assertions are the gate. The `.rma` extension remains private.
