...
export interface BrowserTrackedWorker {
  ...
  snapshot(): Readonly<BrowserAvcWorkerSnapshot>;
  induceFailure(): void;
}

export interface BrowserTrackedRenderer {
  readonly renderer: FrameRenderer;
  snapshot(): Readonly<BrowserAvcRendererSnapshot>;
}

export interface BrowserTrackedPlayback {
  settled(): Promise<void>;
  snapshot(): Readonly<BrowserAvcPlaybackSnapshot>;
}

/** Shared instrumentation/control plane; it never owns candidate resources. */
export class BrowserAvcCandidateHub
  implements BrowserAvcCandidateControls {
  ...
  public registerCandidate(
    ...
    this.#candidateOrder.push(Object.freeze({
      id: rendition.id,
      area: context.candidate.visibleColorArea,
  ...
  public readPixels(): Readonly<BrowserAvcReadPixelsResult> {
  ...
  public snapshot(): Readonly<BrowserAvcCandidateSnapshot> {
    this.#pruneResources();
...
/** @deprecated Use BrowserAvcCandidateHub. */
export { BrowserAvcCandidateHub as BrowserOpaqueCandidateHub };
...
function emptyWorkerSnapshot(): Readonly<BrowserAvcWorkerSnapshot> {
  return Object.freeze({
    metrics: null,
    openFrames: 0,
    pendingRequests: 0,
...
function emptyRendererSnapshot(): Readonly<BrowserAvcRendererSnapshot> {
  return Object.freeze({
    snapshot: null,
    backendAlive: false,
    glResourceCount: 0
...
function emptyPlaybackSnapshot(): Readonly<BrowserAvcPlaybackSnapshot> {
  return Object.freeze({
    scheduler: null,
    cut: null,
    reversible: null,
