...
import {
  DEFAULT_AVC_CANDIDATE_CLOCK,
  DEFAULT_AVC_CANDIDATE_TIMERS,
  DeferredAvcPlaybackSession,
  AvcCandidateOperationControl
} from "./avc-candidate-factory-support.js";
import {
  cloneAvcPresentation
...
export class AvcCandidateAttempt implements IntegratedCandidateAttempt {
  ...
  readonly #playback = new DeferredAvcPlaybackSession();
  readonly #resources: AvcCandidateResources;
  ...
  public get playback(): DeferredAvcPlaybackSession {
  ...
  public async prepare(options: {
    ...
    if (this.#state !== "new") {
      throw new IntegratedPlaybackInvariantError(
        "AVC candidate prepare may run exactly once"
      );
    }
    this.#state = "preparing";
    let control: AvcCandidateOperationControl | null = null;
    try {
  ...
  public async prepareActivation(
    ...
    if (this.#state !== "prepared") {
      throw new IntegratedPlaybackInvariantError(
        "AVC candidate activation requires completed preparation"
      );
    }
    ...
    try {
      ...
      if (!sameGraphPresentation(expected, options.expectedPresentation)) {
        throw new IntegratedPlaybackInvariantError(
          "AVC candidate activation presentation is stale"
        );
      }
      const expectedPresentation = cloneAvcPresentation(
  ...
  public drawInitial(
    ...
    if (
      ...
      throw new IntegratedPlaybackInvariantError(
        "AVC candidate activation token is not owned by this attempt"
      );
    }
    if (!sameGraphPresentation(activation.expectedPresentation, presentation)) {
      throw new IntegratedPlaybackInvariantError(
        "AVC candidate initial draw identity diverged"
      );
    }
    if (this.#initialDrawn) {
      throw new IntegratedPlaybackInvariantError(
        "AVC candidate initial presentation was already consumed"
      );
    }
