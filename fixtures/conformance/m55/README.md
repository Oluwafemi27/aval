...
`opaque-all-routes.avl` is deterministic output from the M5 compiler using 30
...
source-frame, unit, manifest, complete-asset, compiler, and tool hashes;
...
requires identity with the checked `.avl`. Binary and provenance changes must
...
reversal timing, fallback recovery, or display scan-out continuity; those claims
